...
class AI:
    ...
    def __init__(self, window, fruit: "Fruit", snake: "Snake", path, maze, dev: "DeveloperTools", columns, rows) -> None:
        ...
        self.survival = False # Enable snake runaway mode
        self.backwards = None # Checks if snake is going backwards or forwards on cheating
    ...
    def survive(self): # For special cases where the snakes new objective is to not trap itself
        ...
        head_special_cnt, blocked = set(), set()
        ...
        for direction, head in possible_moves.items():
            ...
            if head not in self.body and 0 <= head[0] <= COLUMNS-1 and 0 <= head[1] <= ROWS-1: # Checks if head isn't part of the body or off the grid, to save CPU resources
                ...
                if excepted:
                    ...
                    blockofftest = True # If theres a deadend, it will not be overrideen later on
                    while flag:
                        adjacent_units = set()
                        ...
                        if not len(adjacent_units) and flag: # Adj units == 0 or not possible adj units left
                            ...
                            flag = False
                            if len(visited) == len(vacant_spaces) and blockofftest: # Passed test, snake can go on it's merry way
                                connected_units[direction] = True
                                input("ghegheghieo")
                            else:
                                connected_units[direction] = False
        ...
        if len([i for i in connected_units.values() if i]) and len(head_special_cnt) != 4 and not len(blocked) > 1:
            print(connected_units.values())
        ...
        if not fruit_collide and self.index+1 != len(self.path):
            ...
            if self.index+1 == len(self.path) and self.path[0] not in self.body:
                ...
                if self.path[self.index-1] == (self.x, self.y+1) and self.last_direction != "up" and (self.x, self.y+1) not in self.body: # Down
                    snake.change_dir("down")
                    self.index -= 1
                    self.x, self.y = (self.x, self.y+1)
                    self.last_direction = "down"
                elif self.path[self.index-1] == (self.x, self.y-1) and self.last_direction != "down" and (self.x, self.y-1) not in self.body: # Up
                    snake.change_dir("up")
                    self.index -= 1
                    self.x, self.y = (self.x, self.y-1)
                    self.last_direction = "up"
                elif self.path[self.index-1] == (self.x-1, self.y) and self.last_direction != "right" and (self.x-1, self.y) not in self.body: # Left
                    snake.change_dir("left")
                    self.index -= 1
                    self.x, self.y = (self.x-1, self.y)
                    self.last_direction = "left"
                elif self.path[self.index-1] == (self.x+1, self.y) and self.last_direction != "left" and (self.x+1, self.y) not in self.body: # Right
                    snake.change_dir("right")
                    self.index -= 1
                    self.x, self.y = (self.x+1, self.y)
                    self.last_direction = "right"
                else:
                    if self.last_direction != "up" and (self.x, self.y+1) not in self.body: # Down
                        snake.change_dir("down")
                        self.x, self.y = (self.x, self.y+1)
                        self.index = self.path.index((self.x, self.y))
                        self.last_direction = "down"
                    elif self.last_direction != "down" and (self.x, self.y-1) not in self.body: # Up
                        snake.change_dir("up")
                        self.x, self.y = (self.x, self.y-1)
                        self.index = self.path.index((self.x, self.y))
                        self.last_direction = "up"
                    elif self.last_direction != "right" and (self.x-1, self.y) not in self.body: # Left
                        snake.change_dir("left")
                        self.x, self.y = (self.x-1, self.y)
                        self.index = self.path.index((self.x, self.y))
                        self.last_direction = "left"
                    elif self.last_direction != "left" and (self.x+1, self.y) not in self.body: # Right
                        snake.change_dir("right")
                        self.x, self.y = (self.x+1, self.y)
                        self.index = self.path.index((self.x, self.y))
                        self.last_direction = "right"
    ...
    def check_hamiltonicity(self):
        ...
        connected_units = dict()
        head_special_cnt, blocked = set(), set() # Checks for special case where there are two deadends, counting it as valid
        possible_moves = {"up": (self.x, self.y-1), "down": (self.x, self.y+1), "left": (self.x-1, self.y), "right": (self.x+1, self.y)} # All possible moves for the snake
        for direction, head in possible_moves.items():
            connected_units[direction] = True
            visited = list() # Units already checked
        ...
        if len(head_special_cnt) == 4 and len(blocked) > 1:
            # input("SPECIAL CASE")
            self.survival = True
    ...
    def cheat(self):
        ...
        checkmaze = self.check_hamiltonicity()
        # checkmaze = {"up": True, "down": True, "left": True, "right": True}
        print(f"Hamiltonicity: {checkmaze}\n")
        if len([i for i in checkmaze.values() if not i]) == 4:
            self.survival = True
            # input(f"{[i for i in checkmaze.values() if not i]}, {(self.x, self.y)}")
            self.survive()
            return
        if not fruit_collide and self.index+1 != len(self.path):
        ...
        if not potential_cheats and not self.backwards:
            #if self.index+1 == len(self.path) and self.path[0] in self.body: input(f"SPECIAL CASE {self.index+1 == len(self.path) and self.path[0] not in self.body}, {self.index}, {(self.x, self.y)}") # Special case, ind 0 to last ind
            #  Special case when list ends and is stuck at (0, 1)
            if self.index+1 == len(self.path) and self.path[0] not in self.body:
                print("RESETTING TO (0, 0)")

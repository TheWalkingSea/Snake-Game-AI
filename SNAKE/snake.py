...
class AI:
    ...
    def __init__(self, window, fruit: "Fruit", snake: "Snake", path, maze, dev: "DeveloperTools", columns, rows) -> None:
        ...
        self.whitelist = list() # survival mode, when a move is 50/50, the invalid spot is whitelisted
    ...
    def survive(self):
        ...
        if len(head_special_cnt) == 4 and blocked:
            input(f"SPECIAL CASE {blocked}")
            self.survival = True
    ...
    def cheat(self):
        ...
        if len([i for i in checkmaze.values() if not i]) == 4:
            self.survival = True
            input(f"{checkmaze.values()}, {(self.x, self.y)}")
            # if (self.x+1, self.y) not in self.body: # Right
            #     self.whitelist.append((self.x+1, self.y))
            # if (self.x-1, self.y) not in self.body: # Left
            #     self.whitelist.append((self.x-1, self.y))
            # if (self.x, self.y-1) not in self.body: # Up
            #     self.whitelist.append((self.x, self.y-1))
            # if (self.x, self.y+1) not in self.body: # Down
            #     self.whitelist.append((self.x, self.y+1))

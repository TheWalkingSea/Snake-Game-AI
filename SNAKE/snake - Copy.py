...
# os.environ['SDL_VIDEO_WINDOW_POS'] = "%d,%d" % (-1300, 220) # Centers window when it appears # 0, 30 OR -1300 220
...
class AI:
    ...
    def survive(self): # For special cases where the snakes new objective is to not trap itself
        input("SURVIVAL MODE 1")
        if len([i for i in self.check_hamiltonicity().values() if not i]) == 4:
            ...
            self.survival = False # Disable survival mode
